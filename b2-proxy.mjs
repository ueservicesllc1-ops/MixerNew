...
function buildBandSyncInfo() {
    const ip = getLanIPv4();
    const port = PORT;
    const url = `http://${ip}:${port}/band`;
    return {
        running: true,
        port,
        ip,
...
app.get('/band/api/state', (req, res) => {
    res.json(latestState);
});

app.get('/band/stream', (req, res) => {
    res.writeHead(200, {
...
app.post('/api/band-sync/broadcast', (req, res) => {
    latestState = req.body && typeof req.body === 'object' ? req.body : {};
    const snapshot = { ...latestState };
    for (const { res: clientRes } of sseClients) {
        try {
            sseWrite(clientRes, 'state', snapshot);
        } catch (_) {}
    }
    res.json({ success: true, clients: sseClients.length });
});

app.get('/api/band-sync/info', (req, res) => {
    res.json(buildBandSyncInfo());
});
