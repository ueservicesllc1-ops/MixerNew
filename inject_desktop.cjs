...
c = c.replace(/const unsubAuth = onAuthStateChanged\(auth, \(user\) => \{/, `const unsubAuth = () => {};
        const user = { uid: 'offline_user', email: 'offline@zionstage.com' };
        setCurrentUser(user);
        const fakeAuth = (user) => {`);

// Replace library loading logic with SQLite calls
const regexLib = /if \(user\) \{\s*\/\/\s*Catálogo Global\s*const qGlobal = query\(collection\(db, 'audio_tracks'\), orderBy\('name'\)\);\s*const unsubGlobal = onSnapshot\(qGlobal, \(snapshot\) => \{/;
c = c.replace(regexLib, `if (user) {
                // SQLite Local Library
                if (typeof window !== 'undefined' && window.zionNative) {
                    window.zionNative.getSongs().then(s => setLibrarySongs(s.map(x => ({...x, tracks: x.tracks_json ? JSON.parse(x.tracks_json) : []}))));
                    window.zionNative.getSetlists().then(s => setSetlists(s.map(x => ({...x, songs: x.songs_json ? JSON.parse(x.songs_json) : []}))));
                }
                const unsubGlobal = () => {};
                const unsubSongs = () => {};
                const unsubSetlists = () => {};
...
if (modalStart !== -1) {
    const nextComment = c.indexOf('{/* ── PRELOADER', modalStart);
    if (nextComment !== -1) {
